...
@section('content')
    ...
    <div class="container">
        <h2 class="mb-4">Files for Agent: {{ $agent->displayName }}</h2>
    ...
    @endif --}}
                    ...
                    <tbody>
                        @foreach ($feeds as $file)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('users.files.allow', $file->slug) }}" method="POST"
                                ...
                                <td class="d-flex justify-content-between">
                                    ...
                                    <div>
                                        ...
                                        <form id="deleteForm{{ $file->slug }}"
                                            action="{{ route('users.feed.delete', $file->slug) }}" method="POST"
                                ...
                                <div class="modal-dialog">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            <form method="POST" action="{{ route('users.feed.update', $file->slug) }}">
                                                @csrf
                                                @method('PUT')
        ...
        <div class="pagination-wrapper d-flex justify-content-center mt-4">
            <ul class="pagination">
                <li class="page-item {{ $feeds->onFirstPage() ? 'disabled' : '' }}">
                    <a class="page-link" href="{{ $feeds->previousPageUrl() }}" tabindex="-1"
                        aria-disabled="true">Previous</a>
                </li>
                @foreach ($feeds->getUrlRange(1, $feeds->lastPage()) as $page => $url)
                    <li class="page-item {{ $feeds->currentPage() == $page ? 'active' : '' }}">
                        <a class="page-link" href="{{ $url }}">{{ $page }}</a>
                    </li>
                @endforeach
                <li class="page-item {{ $feeds->hasMorePages() ? '' : 'disabled' }}">
                    <a class="page-link" href="{{ $feeds->nextPageUrl() }}">Next</a>
                </li>
            </ul>
        </div>
