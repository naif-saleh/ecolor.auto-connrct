...
class TokenService
{
    // Method to get the current token, or refresh if expired
    public function getToken()
    {
        // Check if the token is expired in the cache or session (example method)
        $token = Cache::get('three_cx_token');

        if (!$token || $this->isTokenExpired($token)) {
            $token = $this->refreshToken();
            Cache::put('three_cx_token', $token, now()->addMinutes(55)); // Store token for 55 minutes
        }
    ...
    {
        // Token expiry logic
        // Example: Check the token's expiry time, or simply if it exists in the cache
        return false; // Placeholder logic, adjust based on your token structure
    }

    // Refresh the token by calling the API endpoint to get a new one
    private function refreshToken()
    {
        $response = Http::post(config('services.three_cx.token_url'), [
        ...
        if ($response->successful()) {
            ...
            Log::error('Failed to refresh 3CX token: ' . $response->body());
            return null;
        }
    }
}
