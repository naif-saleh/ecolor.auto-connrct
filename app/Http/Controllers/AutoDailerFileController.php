...
class AutoDailerFileController extends Controller
{
    ...
    public function uploadCsv(Request $request)
    {
        ...
        try {
            ...
            // Time and date validation from first row
            $firstDataRow = fgetcsv($handle);
            if ($firstDataRow === false || count($firstDataRow) < 6) {
                fclose($handle);
            ...
            $insertData = [];
            $seenMobiles = [];
            $invalidNumbers = []; // Array to store all invalid numbers (both duplicates and non-numeric)
            $invalidReasons = []; // Array to store reasons why numbers are invalid


            // Process all rows including the first data row
            ...
            if (!empty($insertData)) {
                AutoDailerUploadedData::insert($insertData);
                Log::info('Inserted remaining ' . count($insertData) . ' records.');
            }
            ...
            // Prepare detailed error message for invalid numbers
            ...
            // Delete the uploaded file entry if it exists
